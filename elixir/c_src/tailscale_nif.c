...
static ERL_NIF_TERM tailscale_errmsg_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    if (buf_size <= 0 || buf_size > MAX_ERRMSG_LEN) {
        return make_error_tuple(env, "invalid_buffer_size");
    }
...
static ERL_NIF_TERM tailscale_set_dir_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    // Get the string with proper null termination
    dir_len = enif_get_string(env, argv[1], dir, sizeof(dir), ERL_NIF_LATIN1);
    if (dir_len <= 0) {
    ...
    if (dir_len >= sizeof(dir)) {
        return make_error_tuple(env, "directory_path_too_long");
    }
...
static ERL_NIF_TERM tailscale_set_hostname_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    // Get the string with proper null termination
    hostname_len = enif_get_string(env, argv[1], hostname, sizeof(hostname), ERL_NIF_LATIN1);
    if (hostname_len <= 0) {
        return make_error_tuple(env, "invalid_hostname_string");
    }

    // Check if the string was truncated (hostname_len == sizeof(hostname))
    if (hostname_len >= sizeof(hostname)) {
        return make_error_tuple(env, "hostname_too_long");
    }
...
static ERL_NIF_TERM tailscale_set_authkey_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    // Get the string with proper null termination
    authkey_len = enif_get_string(env, argv[1], authkey, sizeof(authkey), ERL_NIF_LATIN1);
    if (authkey_len <= 0) {
        return make_error_tuple(env, "invalid_authkey_string");
    }

    // Check if the string was truncated
    if (authkey_len >= sizeof(authkey)) {
        return make_error_tuple(env, "authkey_too_long");
    }
...
static ERL_NIF_TERM tailscale_set_control_url_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    // Get the string with proper null termination
    url_len = enif_get_string(env, argv[1], control_url, sizeof(control_url), ERL_NIF_LATIN1);
    if (url_len <= 0) {
        return make_error_tuple(env, "invalid_control_url_string");
    }

    // Check if the string was truncated
    if (url_len >= sizeof(control_url)) {
        return make_error_tuple(env, "control_url_too_long");
    }
...
static ERL_NIF_TERM tailscale_dial_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    int network_len, addr_len;

    if (!enif_get_resource(env, argv[0], tailscale_resource_type, (void **)&handle)) {
        return make_error_tuple(env, "invalid_tailscale_handle");
    }

    // Get network string with proper null termination
    network_len = enif_get_string(env, argv[1], network, sizeof(network), ERL_NIF_LATIN1);
    if (network_len <= 0) {
        return make_error_tuple(env, "invalid_network_string");
    }

    // Check if network string was truncated
    if (network_len >= sizeof(network)) {
        return make_error_tuple(env, "network_string_too_long");
    }

    // Get address string with proper null termination
    addr_len = enif_get_string(env, argv[2], addr, sizeof(addr), ERL_NIF_LATIN1);
    if (addr_len <= 0) {
        return make_error_tuple(env, "invalid_addr_string");
    }

    // Check if address string was truncated
    if (addr_len >= sizeof(addr)) {
        return make_error_tuple(env, "addr_string_too_long");
    }
...
static ERL_NIF_TERM tailscale_getips_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    // Ensure buffer size is reasonable
    if (buf_size <= 0 || buf_size > MAX_ADDR_LEN) {
        return make_error_tuple(env, "invalid_buffer_size");
    }
...
static ERL_NIF_TERM tailscale_loopback_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
    ...
    char proxy_cred_out[33];  // 32 chars + null terminator
    char local_api_cred_out[33];  // 32 chars + null terminator
    ...
    memset(proxy_cred_out, 0, sizeof(proxy_cred_out));
    memset(local_api_cred_out, 0, sizeof(local_api_cred_out));
    ...
    proxy_cred_out[sizeof(proxy_cred_out) - 1] = '\0';
    local_api_cred_out[sizeof(local_api_cred_out) - 1] = '\0';
...
ERL_NIF_INIT(Elixir.Tailscale.NIF, nif_funcs, load, NULL, upgrade, unload)
